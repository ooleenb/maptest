...
# 端点: 标量场二进制
# ============================================================
# 变量名安全校验: 只允许字母/数字/下划线, 防止路径穿越 (../ 之类)
import re as _re
_VALID_VAR_PATTERN = _re.compile(r"^[A-Za-z0-9_]+$")


@app.get("/api/sources/{source}/days/{date}/scalar/{var}")
async def get_scalar_data(source: str, date: str, var: str):
    """
    返回某日某变量的 Float32 二进制数据。
    
    变量集是动态的, 取决于数据源:
      - ROMS (perth/cwa):   temp / salt / zeta
      - WRF  (wrf_d01/d02): temp / Pair / Qair / rain / cloud
    
    这里不再硬编码白名单 —— 而是: 先确保该日已 prep, 然后看
    {var}.bin 文件是否存在. 存在就返回, 不存在返回 404 并附上
    该日实际可用的变量列表 (帮助前端调试).
    
    布局: n_frames 帧 × n_eta × n_xi 个 float32, little-endian
    """
    _validate_source(source)
    _validate_date(date)
    
    # 变量名格式校验 (防路径穿越攻击, 不是业务白名单)
    if not _VALID_VAR_PATTERN.match(var):
        raise HTTPException(
            status_code=400,
            detail=f"Invalid variable name: {var!r}"
        )
    ...
    if not bin_path.exists():
        # 列出该日实际有哪些变量 (扫描 .bin 文件)
        available = sorted(
            p.stem for p in day_dir.glob("*.bin")
        )
        raise HTTPException(
            status_code=404,
            detail=(
                f"Variable {var!r} not available for source {source!r} "
                f"on {date}. Available: {', '.join(available) or '(none)'}"
            )
        )
